...
static const az_span command_name_recv_message_span = AZ_SPAN_LITERAL_FROM_STR("recvMsg");
    ...
    az_iot_hub_client_command_request const* command_request)
{
    ...
    // Check for 'recvMsg' command
    else if ((az_span_is_content_equal(command_request->component_name, AZ_SPAN_EMPTY))
        && (az_span_is_content_equal(command_name_recv_message_span, command_request->command_name)))
    {
        az_iot_status status;
        az_span command_response_payload = AZ_SPAN_FROM_BUFFER(command_response_payload_buffer);

        // Process the command
        if (!COMMAND_Handler_recvMsg(message_span, command_response_payload, &command_response_payload))
        {
            status = AZ_IOT_STATUS_BAD_REQUEST;
        }
        else
        ...
        EWF_LOG("Client processed command 'recvMsg'\r\n");
