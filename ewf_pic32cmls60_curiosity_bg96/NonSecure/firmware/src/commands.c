...
static const az_span command_recvMsg_payload_span            = AZ_SPAN_LITERAL_FROM_STR("recvMsgString");
static const az_span command_resp_empty_recvMsg_payload_span = AZ_SPAN_LITERAL_FROM_STR("Message string is empty. Specify string.");
static const az_span command_resp_alloc_error_recvMsg_span   = AZ_SPAN_LITERAL_FROM_STR("Failed to allocate memory for the message.");
 ...
 **********************************************/
az_result COMMAND_Handler_recvMsg(
    az_span   payload_span,
    az_span   response_span,
    az_span*  out_response_span)
{
    ...
    if (az_span_size(payload_span) > 0)
    {
        ...
        while (jr.token.kind != AZ_JSON_TOKEN_END_OBJECT)
        {
            ...
            {
                if (az_json_token_is_text_equal(&jr.token, command_recvMsg_payload_span))
                {
    ...
    if (spanSize == -1)
    {
        ...
        ret = build_command_error_response_payload(response_span,
                                                   command_resp_empty_recvMsg_payload_span,
                                                   out_response_span);
    }
    else if ((messageString = malloc(spanSize + 1)) == NULL)
    {
        ...
        ret = build_command_error_response_payload(response_span,
                                                   command_resp_alloc_error_recvMsg_span,
                                                   out_response_span);
    }
    else
    {
